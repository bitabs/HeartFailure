...
export default class UserInfo extends Component {
  ...
  componentDidMount() {
    ...
    if (this._isMounted) this.initialiseDB();
  }
  ...
  initialiseDB = () => {
    ...
    const {User, userRef, PCRef, DCRef, healthRef, uid, type, loggedInUser, CurrentUserUID, userView, updateIndex} = this.props;
    Database.initialiseMessagesDB(loggedInUser.name, CurrentUserUID, User.uid, type, type === "Patient" ? PCRef : DCRef, healthRef).catch(e => console.log(e));

    PCRef.on('value', snap => {
      this.fetchComments([PCRef, DCRef]).then(refValues => {
        const [PatientsComments, DoctorsComments] = refValues;
        this.filterMsg(this.props.User, {...PatientsComments, ...DoctorsComments});
      })
    });
  ...
  sendMessage = (toUid) => {
    ...
    const
      { message }                  = this.state,
      { type, uid, PCRef, DCRef } = this.props;

    if (this._isMounted)
      Database.setMessage(uid, toUid, type === "Patient" ? PCRef : DCRef, message);
  };
  ...
  filterMsg = (User, AsyncMessages) => {
    const { uid, CurrentUserUID } = this.props;
    let filtered = Object.keys(AsyncMessages).reduce((acc, val) => {
      const patientToDoctor = `${CurrentUserUID}<=>${User.uid}`;
      const doctorToPatient = `${User.uid}<=>${CurrentUserUID}`;
