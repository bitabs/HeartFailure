...
export default class UserBox extends Component {
  ...
  render() {
    ...
    const patients = User && User.Patients ? Object.keys(User.Patients).map((uid, i) => {
      if (i < 3) return (
          ...
          <View style={styles.rightContainer}>
            ...
            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
              ...
              <View style={{alignSelf: 'flex-start', marginTop: 5, flexDirection: 'row', alignItems: 'center'}}>
                ...
                <Text numberOfLines={1} style={{fontSize: 10, color: '#909aae', flexWrap: 'wrap', maxWidth: 100}}>{User.address || "Not Specified"}</Text>
              </View>
            </View>
            ...
            <View style={{marginTop: 10}}>
              <TouchableOpacity
                activeOpacity={1}
                onPress={() => this.update({uid: this.props.uid,...this.props.User})}
                style={[styles.messageBtn, {padding: 15, borderRadius: 5, paddingTop: 10, paddingBottom: 10, backgroundColor: "#f6f6f6",}]}
              >
